...
class MealSchedule
  ...
  #         has no overlapping steps. In this context map Integers to single
  #         Steps, not Arrays of Steps.
  #
  # Returns an Array of Hashes from Integer start times to Steps.
  def collimated_schedule
    # The array we're populating to return. A column is one schedule.
    columns = []
    # A schedule of the steps that have no yet been put into a column.
    ...
    until remainder.empty?
      ...
      remainder.keys.sort.each do |time|
        if time >= next_open_time
          ...
          # Add the longest-time step at this time to the column.
